...
export default function MobileEmployeesEditor({
  ...
  const [edited, setEdited] = useState(() => {
    ...
    return {
      ...
      rateKBS: parseFloat(employee['Rate KBS']) || 0,
      rateLGM: parseFloat(employee['Rate LGM']) || 0,
  ...
  useEffect(() => {
    const fullName = `${edited.first_name || ''} ${edited.last_name || ''}`.trim();
    if (fullName && fullName !== edited.nombre) {
      setEdited(prev => ({ ...prev, nombre: fullName }));
    }
  }, [edited.first_name, edited.last_name]);
    ...
    <>
      ...
      <div className="space-y-4 pb-8">
        ...
        <div>
          ...
          <input value={edited.nombre} readOnly className="w-full h-10 bg-gray-100 rounded-xl px-3.5 text-sm text-gray-400 outline-none" />
        </div>
